...
async def lifespan(app: FastAPI):
    logger.info(f"Loading ML model from MLflow URI: {settings.api.mlflow_model_uri}")
    try:
        ml_models["ids_model"] = mlflow.pyfunc.load_model(model_uri=settings.api.mlflow_model_uri)
        logger.info("ML model loaded successfully and is available.")
    except Exception as e:
        logger.error(f"Failed to load ML model on startup: {e}", exc_info=True)
...
def predict(
    ...
    input_df = pd.DataFrame([record.model_dump() for record in prediction_input.records])
    ...
    if settings.api.model_mode == 'binary':
        ...
        label_mapping = {
            'Normal': 0, 'Analysis': 1, 'Backdoor': 2, 'DoS': 3, 'Exploits': 4,
    ...
    for index, record in enumerate(prediction_input.records):
        ...
        crud.create_prediction_with_record(
            db=db,
            record_in=record,
            result_in=prediction_result
        )
        results.append(prediction_result)

    logger.info(f"{len(results)} predictions were successfully processed and stored.")
    return schemas.PredictionResponse(predictions=results)

@app.get("/predictions/", response_model=List[schemas.PredictionResult], tags=["Predictions"])
...
@app.get("/predictions/{prediction_id}", response_model=schemas.PredictionResult, tags=["Predictions"])
def read_prediction(
    prediction_id: int,
    db: Session = Depends(get_db)
):
